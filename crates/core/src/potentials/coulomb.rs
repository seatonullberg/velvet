...
pub trait CoulombPotential: Potential {
    fn energy(&self, qi: f32, qj: f32, r: f32) -> f32;
    fn energy_self(&self, qi: f32) -> f32;
    fn force(&self, qi: f32, qj: f32, r: f32) -> f32;
}
...
impl CoulombMeta {
    pub fn new(cutoff: f32, system: &System) -> CoulombMeta {
...
impl Wolf {
    pub fn new(cutoff: f32) -> Wolf {
        let alpha = PI / cutoff;
        let alpha_cut = alpha * cutoff;
        let alpha_cut_2 = alpha_cut * alpha_cut;
        let energy_constant = libm::erfcf(alpha_cut) / cutoff;
        let force_constant = libm::erfcf(alpha_cut) / (cutoff * cutoff)
...
impl CoulombPotential for Wolf {
    fn energy(&self, qi: f32, qj: f32, r: f32) -> f32 {
        qi * qj * (libm::erfcf(self.alpha * r) / r - self.energy_constant) / FOUR_PI_EPSILON_0
    }
    ...
    fn force(&self, qi: f32, qj: f32, r: f32) -> f32 {
        ...
        let exp_alpha_r = f32::exp(-alpha_r * alpha_r);
        let factor = libm::erfcf(alpha_r) / r2 + self.alpha * FRAC_2_SQRT_PI * exp_alpha_r / r;
...
mod tests {
    ...
    fn wolf() {
        ...
        let energy_target = -0.09263977 as f32;
        assert_relative_eq!(
