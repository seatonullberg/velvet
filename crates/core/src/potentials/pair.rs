...
impl PairMeta {
    pub fn new(cutoff: f64, elements: (Element, Element), system: &System) -> PairMeta {
...
impl Harmonic {
    ...
    pub fn new(k: f64, x0: f64) -> Harmonic {
        Harmonic { k, x0 }
    }
}
...
impl Mie {
    ...
    pub fn new(epsilon: f64, sigma: f64, gamma_a: f64, gamma_r: f64) -> Mie {
        Mie {
            epsilon,
            sigma,
            gamma_a,
...
impl Morse {
    ...
    pub fn new(a: f64, d_e: f64, r_e: f64) -> Morse {
        Morse { a, d_e, r_e }
    }
}
...
impl PairPotential for Morse {
    #[inline]
    fn energy(&self, r: f64) -> f64 {
        let term_a = f64::exp(-2.0 * self.a * (r - self.r_e));
        let term_b = 2.0 * f64::exp(-self.a * (r - self.r_e));
        self.d_e * (term_a - term_b)
    }

    #[inline]
    fn force(&self, r: f64) -> f64 {
        let term_a = f64::exp(-self.a * (r - self.r_e));
        let term_b = f64::exp(-2.0 * self.a * (r - self.r_e));
...
mod tests {
    ...
    fn harmonic() {
        ...
        let r0_energy = 12.5;
        let r0_force = -50.0;
        assert_relative_eq!(r0_energy, harmonic.energy(r0), epsilon=1e-6);
        assert_relative_eq!(r0_force, harmonic.force(r0), epsilon=1e-6);

        // test r1 energy and force
        let r1_energy = 0.0;
        let r1_force = 0.0;
        assert_relative_eq!(r1_energy, harmonic.energy(r1), epsilon=1e-6);
        assert_relative_eq!(r1_force, harmonic.force(r1), epsilon=1e-6);

        // test r1 energy and force
        let r2_energy = 12.5;
        let r2_force = 50.0;
        assert_relative_eq!(r2_energy, harmonic.energy(r2), epsilon=1e-6);
        assert_relative_eq!(r2_force, harmonic.force(r2), epsilon=1e-6);
    }
    ...
    fn lennard_jones() {
        ...
        assert_relative_eq!(r0_energy, lj.energy(r0), epsilon=1e-6);
        assert_relative_eq!(r0_force, lj.force(r0), epsilon=1e-6);

        // test r1 energy and force
        let r1_energy = 0.0;
        let r1_force = -9.6;
        assert_relative_eq!(r1_energy, lj.energy(r1), epsilon=1e-6);
        assert_relative_eq!(r1_force, lj.force(r1), epsilon=1e-6);

        // test r2 energy and force
        let r2_energy = -0.89096529;
        let r2_force = 0.8846772;
        assert_relative_eq!(r2_energy, lj.energy(r2), epsilon=1e-6);
        assert_relative_eq!(r2_force, lj.force(r2), epsilon=1e-6);
    }
    ...
    fn mie() {
        ...
        let r0_energy = 42.948872;
        let r0_force = -303.469598;
        assert_relative_eq!(r0_energy, mie.energy(r0), epsilon=1e-6);
        assert_relative_eq!(r0_force, mie.force(r0), epsilon=1e-6);

        // test r1 energy and force
        let r1_energy = 0.0;
        let r1_force = -9.6;
        assert_relative_eq!(r1_energy, mie.energy(r1), epsilon=1e-6);
        assert_relative_eq!(r1_force, mie.force(r1), epsilon=1e-6);

        // test r2 energy and force
        let r2_energy = -0.89096529;
        let r2_force = 0.8846772;
        assert_relative_eq!(r2_energy, mie.energy(r2), epsilon=1e-6);
        assert_relative_eq!(r2_force, mie.force(r2), epsilon=1e-6);
    }
    ...
    fn morse() {
        ...
        assert_relative_eq!(r0_energy, morse.energy(r0), epsilon=1e-6);
        assert_relative_eq!(r0_force, morse.force(r0), epsilon=1e-6);

        let r1_energy = -4.0;
        let r1_force = 0.0;
        assert_relative_eq!(r1_energy, morse.energy(r1), epsilon=1e-6);
        assert_relative_eq!(r1_force, morse.force(r1), epsilon=1e-6);

        let r2_energy = -2.8864117;
        let r2_force = 2.9908366;
        assert_relative_eq!(r2_energy, morse.energy(r2), epsilon=1e-6);
        assert_relative_eq!(r2_force, morse.force(r2), epsilon=1e-6);
    }
}
